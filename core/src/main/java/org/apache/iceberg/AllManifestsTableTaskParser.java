...
class AllManifestsTableTaskParser {
  ...
  static void toJson(AllManifestsTable.ManifestListReadTask task, JsonGenerator generator)
    ...
    generator.writeStringField(MANIFEST_LIST_LOCATION, task.manifestListFile().location());
    if (task.manifestListFile().encryptionKeyID() != null) {
      generator.writeStringField(MANIFEST_LIST_KEY_ID, task.manifestListFile().encryptionKeyID());
    }
